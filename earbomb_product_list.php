...
//$sql_query =sprintf("SELECT * FROM `product` LIMIT %s, %s", ($page-1)*$per_page, $per_page);
...
//    利用SQL語法算出資料總筆數
    ...
    $t_sql = "SELECT COUNT(1) FROM `product` WHERE `category_sid`=1";
    $total_result = $mysqli->query($t_sql);
    $total_rows = $total_result->fetch_row()[0];
    $pages = ceil($total_rows/$per_page);
...
//    使用sprintf因為頁首開始有其規律為8的倍數 每頁起始值應為(n-1)*8用%s放入字串
//    SQL中LIMIT第一個值為起始值以","作分隔 第二個值為從起始值開始抓出多少數量的值
    $sql_query =sprintf("SELECT * FROM `product` WHERE `category_sid`=1 LIMIT %s, %s", ($page-1)*$per_page, $per_page);
    $result = $mysqli->query($sql_query);
    $total_records = $result->num_rows;
        ...
        <div class="list-row">
            ...
            <nav aria-label="Page navigation example" class="pagination">
                <ul class="pagination">
                    ...
                    <li class="page-item <?= $page==$i ? 'active' : '' ?> <?= $page==$i ? 'disabled' : '' ?>"><a class="page-link" href="?page=<?= $i ?>"><?= $i ?></a></li>
                    <?php endfor; ?>
                    <li class="page-item  <?= $page==$pages ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=<?= $page<$pages ? $page+1 : $pages ?>" aria-label="Next">
